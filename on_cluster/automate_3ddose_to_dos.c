...
BUT unlike its namesake (minus 'automate'), this is gonna be automated such that it'll run it on a given number (maxSubmit) of files at a time (in much the same way as automateDosxyznrcSubmit.c)
*/
...
main (int argc, char** argv)
{
  ...
  int maxSubmit = 100;  //maximum number of files to convert at a time


  //enter arguments when running like:
  // ./auto2ddt2dos 3ddose_file_base dos_file_base totnumbeams startingBeam startingBeamlet
  ...
  int n, l;
  int lstart = atoi(argv[5]);
  int numSubmitted = 0;
  
  for(n = atoi(argv[4]); n <= atoi(argv[3]); n++) {
    for(l = lstart; l <= numBeamlets; l++) {
      char name_3ddose[256];
      sprintf(name_3ddose,"%sBeam%dBeamlet%d.3ddose",argv[1],n,l);
      printf("uhh %s\n",name_3ddose);
      
      //do the conversion if the file exists; otherwise go on to the next one
      if(fileExists(name_3ddose)==1) {
        ...
        char name_dos[256];
        //naming format is: filebaseBeamNBeamletL.dos
        sprintf(name_dos,"%sBeam%dBeamlet%d.dos",argv[2],n,l);
